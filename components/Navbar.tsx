...
export default function Navbar() {
  ...
  // iOS WebKit requires position:fixed to prevent scroll-through — plain
  // overflow:hidden on body doesn't work reliably and can crash some iOS tabs.
  useEffect(() => {
    if (menuOpen) {
  ...
  return (
    <header
      className={`site-header fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled || menuOpen
          ? "bg-white shadow-md md:bg-white/[0.98] md:backdrop-blur-xl"
          : "bg-transparent"
      }`}
    ...
    >
      ...
      {/* Mobile menu: no height:auto animation — that pattern crashes some iOS WebKit tabs */}
      {menuOpen ? (
        <div
          id="mobile-menu"
